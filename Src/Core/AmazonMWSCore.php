...
class AmazonMWSCore
{
	...
	public function getResultAsObject($xml = "")
    {
        $xmlParsedCore = new \AmazonMWS\Core\AmazonResponseCore();

        if(!empty($xml))
            return $xmlParsedCore->getParsedXML($xml);

        return $xmlParsedCore->getParsedXML($this->getResult());
